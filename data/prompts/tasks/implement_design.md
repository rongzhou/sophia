...
Use the deterministic scaffold below as structural assistance, not as permission for the tool to invent semantics. It reduces mechanical burden but does not implement the algorithm for you. You are responsible for translating the .pseudo semantics into valid, checkable Sophia v0 source. Preserve only the explicit contract entries listed in `structure_plan.action_contract_hints`: file paths, top-level names, capability binding, explicitly typed input fields, explicitly typed output field, explicitly declared states, explicitly typed entity fields, and explicit effects. If an input/output/entity field is absent from that plan, choose the smallest compiled contract that faithfully represents the .pseudo semantics; do not add, remove, reorder, strengthen, weaken, or reinterpret algorithm steps. Do not treat scaffold placeholder comments or placeholder `result: Unit` as a required contract. When the .pseudo contains "subaction Name" blocks, treat those names as required helper action boundaries: create helper action files with those names inside the same domain layout and invoke them with direct action expressions from the main scaffold action. Replace TODO/tool comments with valid Sophia v0 source. Do not leave scaffold comments in the final files.
