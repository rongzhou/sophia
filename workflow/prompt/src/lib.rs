...
pub fn design_system_prompt() -> String {
    "你是工作流的设计者。只输出一个 JSON 对象，严格符合 design_result schema：\
     purpose（字符串）、pseudocode（字符串）、libraries（字符串数组，从库目录选用的标准库名；\
     不用库则空数组）。pseudocode 是**单个**结构化伪代码文档（可在其中用文字描述多个业务部分），\
     **不是**代码、声明、类型签名、文件数组或文件布局。pseudocode 必须是结构化算法文档：\
     第一行写 `<!-- sophia-pseudo: v1 -->`，并包含六个固定 heading：Purpose、Inputs、Outputs、\
     Algorithm、Constraints、Forbidden。只描述算法和可观察行为；不要输出 markdown 围栏。"
        .to_string()
}
