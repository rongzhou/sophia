...
export type SophiaParseDiagnostic = Diagnostic;
...
export interface ParseSophiaFileResult {
  ...
  diagnostics: SophiaParseDiagnostic[];
}
...
export function parseSophiaSource(content: string, filePath = "<source>"): ParseSophiaFileResult {
  const diagnostics: SophiaParseDiagnostic[] = [];
  const topLevel = parseSophiaTopLevelDeclarations(content);

  if (!hasBalancedSophiaBraces(content)) {
    diagnostics.push(error("PARSE-SYNTAX-001", filePath, "Unbalanced braces in Sophia source."));
  }

  if (topLevel.length === 0) {
    diagnostics.push(error("PARSE-FILE-001", filePath, "No top-level Sophia node found."));
  }

  if (topLevel.length > 1) {
    diagnostics.push(
      error("PARSE-FILE-002", filePath, `Expected one top-level Sophia node, found ${topLevel.length}.`),
    );
  }
