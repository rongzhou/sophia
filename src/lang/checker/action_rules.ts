import { flattenSophiaBodyStatements, parseSophiaBody } from "../body/ast.js";
import { buildBodyStatementTypeEnvironments } from "../body/return.js";
import { checkVariableLifecycle } from "../body/lifecycle.js";
import { checkReturnShape } from "../body/return.js";
import { parseActions } from "../ast/check_model.js";
import { errorDiagnostic } from "../ast/diagnostics.js";
import { parseSophiaFieldDeclarations, parseSophiaStorageEffect } from "../ast/signature.js";
...
import { inferSophiaExpressionType } from "../ast/expression.js";
import { parseSophiaIntentType, unwrapSophiaIntentType } from "../ast/types.js";
    ...
    if (context.actionNames.has(action.name)) {
      context.diagnostics.push(
        errorDiagnostic(
          "CHECK-ACTION-004",
          filePath,
          `Duplicate action declaration: ${action.name}.`,
        ),
      );
    }
  ...
  if (!action.hasCapability) {
    context.diagnostics.push(
      errorDiagnostic(
        "CHECK-ACTION-001",
        filePath,
        `Action ${action.name} is missing capability binding.`,
      ),
    );
  }
  if (!action.hasEffectsBlock) {
    context.diagnostics.push(
      errorDiagnostic(
        "CHECK-ACTION-002",
        filePath,
        `Action ${action.name} is missing effects block.`,
      ),
    );
  }
  if (!action.hasBody) {
    context.diagnostics.push(
      errorDiagnostic("CHECK-ACTION-003", filePath, `Action ${action.name} is missing body block.`),
    );
  }
  if (!action.hasOutputBlock) {
    context.diagnostics.push(
      errorDiagnostic(
        "CHECK-ACTION-005",
        filePath,
        `Action ${action.name} is missing output block.`,
      ),
    );
  }
