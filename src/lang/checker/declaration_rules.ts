...
import { errorDiagnostic, type Diagnostic } from "../ast/diagnostics.js";
import { parseSophiaImmediateNamedBlocks, parseSophiaTopLevelDeclarations } from "../ast/parser.js";
import { isSophiaStorageEffect, parseSophiaEffectNames } from "../ast/signature.js";
import { isSupportedSophiaType, type SophiaEntityTypes } from "../ast/types.js";
...
export function checkStateDeclarations(
  ...
  for (const state of parseStateDeclarations(content)) {
    if (context.stateNames.has(state.name)) {
      context.diagnostics.push(
        errorDiagnostic("CHECK-STATE-001", filePath, `Duplicate state declaration: ${state.name}.`),
      );
    }
      ...
      if (seenValues.has(value)) {
        context.diagnostics.push(
          errorDiagnostic(
            "CHECK-STATE-003",
            filePath,
            `Duplicate state value declaration: ${value}.`,
          ),
        );
      }
    ...
    if (context.errorNames.has(declaration.name)) {
      context.diagnostics.push(
        errorDiagnostic(
          "CHECK-ERROR-001",
          filePath,
          `Duplicate error declaration: ${declaration.name}.`,
        ),
      );
    }
