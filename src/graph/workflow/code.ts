...
import type { GraphEdge, GraphEdgeType } from "../core/nodes.js";
...
async function createResultNode(options: {
  ...
  const node = await options.store.createNode({
    ...
    goal: options.sourceNode.goal,
    summary: options.summary,
    artifacts: ["result.json"],
    tags: options.tags,
  });
  await options.store.writeArtifactJson(node, "result.json", options.resultArtifact ?? options.result);
