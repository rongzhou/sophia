...
`baseline` mode 执行 LLM 生成的 Python，需 `python3`（缺失时自动只跑 `sophia` mode）。产物落 `sophia-runs/benchmark/<label>/{runs.jsonl,summary.md}`（成功率 / 耗时两项核心指标）。批量串行执行见 `scripts/run_benchmark.sh`。详见 `docs/cn/benchmark_test.md`。
...
版本对齐是硬约束（见 `docs/cn/engineering_notes.md`）：tree-sitter crate、CLI、生成的 `parser.c` ABI 必须一致。
