...
感谢有意参与 Sophia。本文档说明开发流程与代码规范。构建与测试见 [INSTALL-CN.md](INSTALL-CN.md)。
...
详见 `docs/cn/engineering_notes.md`（决策日志，含上述原则的完整表述）。
...
4. **同步文档**：更新 `docs/cn/dev_checklist_v1.md`（当前进展 SSOT + 变更记录）；涉及决策时在 `docs/cn/engineering_notes.md` 新增条目；涉及图 schema 时更新 `docs/cn/workflow_graph_spec.md`。
5. **验证**（必须全绿）：
...
- 真实 LLM 的端到端测试是 `example`，**不进** CI，按需手动运行（见 `docs/cn/e2e_test.md`）。
- 快照测试用 [`insta`](https://insta.rs/)：新增 / 变更后用 `cargo insta review` 审阅并接受 `.snap.new`。
- 防答案泄漏是 e2e 的第一原则：任务答案不得进入共享脚手架（语法基线 / system prompt）。

## 修改语法

改动 `core/syntax/grammar.js` 须用对齐版本的 Tree-sitter CLI 重新生成 `parser.c`（见 INSTALL-CN.md「修改语法」）。版本对齐（crate / CLI / ABI 三者一致）是硬约束。
