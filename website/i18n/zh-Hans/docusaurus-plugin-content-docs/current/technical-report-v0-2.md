---
title: 技术报告 v0.2
sidebar_label: 技术报告 v0.2
slug: /technical-report-v0-2
---

# Sophia：代码预训练之外的 LLM-native 图编程路径
...
v0.2 的意义不是证明 Sophia 在 benchmark 成功率上胜过 direct TypeScript。当前 benchmark 只说明这个替代路径已经可运行、可记录、可复现，并覆盖 `Optional`、`state`、effect、entity 和跨 action pipeline 等最小语言面。Sophia 的长期价值在于证明：编程能力不必完全来自代码预训练的内化，也可以部分来自为 LLM 重新设计的程序表示和外部化工程纪律。
...
3. 给出 v0.2 原型实现，覆盖确定性 checker、TypeScript backend、runtime validation、action-rooted context、constraint audit、strip-assist 等价和 graph repair/materialize gate。
4. 给出当前 benchmark 运行记录，作为可运行性和语言面覆盖的证据，同时明确它不是 Sophia 的核心价值证明。
...
v0.2 是一个收束后的可测试原型边界，不是完整 Sophia。
...
v0.2 的一个重要修正是移除 design 阶段的 formal syntax 污染。伪代码生成阶段不再接收 Sophia type/effect syntax、source paths、implementation labels、formal scaffold 或 `program { ... }` 风格伪 DSL。implementation 阶段可以接收 deterministic structure plan，但该 plan 只固定显式 contract、路径、命名、state/effect 等结构，不生成业务算法。

---

## 6. ASG 与 action-rooted context

Sophia 的语义模型是 ASG。v0.2 用文件系统实现：一个顶层语义节点一个 `.sophia` 文件，domain-first 布局。
...
未来 `task` 顶层节点会把 closure root 从 action 扩展到 task；v0.2 只承诺 action-rooted context。

---

## 7. 外部化编程纪律示例：Intent Types

Sophia 的主张不是只靠某一个类型特性成立；Intent Types 是“把编程能力外部化”的一个代表性例子。它展示了 Sophia 如何把模型容易忘记的语义历史，从对话记忆或命名约定中拿出来，变成机器可检查的语言事实。

v0.2 已实现的 intent 边界包括：
...
v0.2 已实现 TypeScript artifact 等价门禁。更完整的未来形态应比较 IR hash 和 codegen artifact hash。这个机制把“自然语言只是辅助”从口号变成机器可检查属性，也是 Sophia 区分 prompt DSL / 注释驱动编程的重要边界。
...
## 11. Benchmark 在 v0.2 中的正确位置

v0.2 重新跑了 Sophia full workflow 与 direct TypeScript baseline。当前 suite 共 16 个任务：

| 层级       | 任务                                                                                          |
| ---------- | --------------------------------------------------------------------------------------------- |
| L1         | `return_constant`、`double_input`、`concat_text`、`is_positive`、`add_two_numbers`            |
| L2         | `sum_first_five`、`countdown_five`、`build_three_numbers`、`first_five_squares`、`rabbit_ten` |
| L3         | `zero_or_positive`、`zero_or_positive_label`、`optional_label_default`、`state_status_label`  |
| category_a | `account_pipeline`、`item_delta_pipeline`                                                     |
...
1. v0.2 full workflow 已经可以端到端执行；
2. benchmark runner、hidden verifier、JSONL 记录和 summary 已经可用；
3. 当前 language surface 已覆盖 `Optional`、`state`、effect、entity 和跨 action pipeline 的最小样本。

它**不能**作为 Sophia 的核心价值证明。尤其不能把论文写成“Sophia 比 direct-ts 成功率更高”。这种论证会把 Sophia 拉回传统 coding-agent 评测框架，忽略语言本身的定性差异。

direct-ts 的两个失败仍可作为诊断样本记录：

- `build_three_numbers`：TypeScript typecheck 通过，但返回 `[2, 4, 6, 8]` 而非 `[2, 4, 6]`；
- `optional_label_default`：TypeScript typecheck 通过，但对 present optional text 返回 `null`。

这些样本说明 hidden verifier 有用，也说明 Sophia workflow 能保留某些语义约束。但它们不是主论点。真正的主结果应服务两条更大的证明：弱代码模型是否能在 Sophia substrate 上完成编程，以及图语言/节点工作流是否能提供传统人类语言范式之外的可行路径。
...
把 `tests/lang/v0_2_regression.test.ts` 中的 Raw/Secret/DB.Write/Console/capability/error propagation fixture 转成 adversarial benchmark。目标不是 Sophia 运行成功，而是 Sophia 静态拒绝不安全 candidate，同时 TypeScript + typecheck + 非对抗性测试 baseline 会接受。
...
Sophia v0.2 的价值不是“在 16 个小任务上 full workflow 比 direct TypeScript 多通过 2 个”。这个结果只能说明原型能跑，不能说明语言为什么必须存在。

Sophia 的真正主张有两层。第一，代码预训练是有价值的捷径，但并非 LLM 获得编程能力的绝对必要条件；编程能力的一部分可以由通用语义理解模型与外部语言、检查器和工作流共同承担。第二，传统人类编程语言范式不是唯一可能的编程 substrate；一个突破线性源码中心、面向 LLM 理解的图编程语言，以及与之配套的启发式节点工作流，是可行的。

Intent Types、capability/effect、error propagation、strip-assist 等价、action-rooted context 和 development graph 是这个方向的最小可运行雏形。后续证明 Sophia 的方式，应围绕“弱代码模型能否借助 Sophia substrate 编程”和“图语言/节点工作流能否外部化传统代码预训练内化的工程纪律”展开，而不是追求在普通 direct-ts benchmark 上全面胜出。
