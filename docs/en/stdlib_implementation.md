...
- Standard-library hosts (`sophia-stdlib::native_host`): `register_native_hosts(&mut host, project_root) -> Result` registers real `reqwest` `Http.Get` and `std::fs` `File.Read`/`File.Write`; file hosts accept relative paths only, reject absolute paths and `..`, and verify the real path remains under `project_root`; `mock_host()` / `register_mock_hosts` register deterministic in-memory buckets (`MockBuckets::seed_http`/`seed_file` for tests/diff tests).
...
- CLI `sophia run` assembly (`commands::run_interpreter_action` / WASM backend): first `register_wasm_library_hosts` (no-op if none), then register native hosts on demand via `register_native_hosts(&mut host, root)` per entry action’s declared effects (`Http.Get`/`File.*`); third-party WASM ops are often `effectful=false` (not reflected in effect declarations), so their hosts are registered unconditionally.
