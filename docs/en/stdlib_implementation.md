...
Library Sophia nodes are registered under domains equal to library names (isolation). User→library references (e.g., `SophiaDigest`) are exempted via `AsgIndex.library_domains` (populated by `with_libraries` from `registry.sophia_sources()`) and resolver’s `check_cross_domain_domain` skipping `ImplicitCrossDomain` for library domains (user↔user remains checked). Library nodes are statically checked like user code (no privilege)—the exemption only affects visibility.
...
- Execution entry: `run_action` (empty registry: only Console/pure logic) vs `run_action_with_host(.., &mut HostRegistry)` (inject library hosts).
...
- Third-party WASM hosts (`sophia-stdlib::register_wasm_library_hosts(&mut host, &registry)`): iterate libraries in the registry that carry `host.wasm` bytes (WASM-effect libs), and register a `runtime::WasmHostFn` per effect-op—internally a `wasmi` instance of `host.wasm` forwarding through the unified byte ABI; `host.wasm` must export `memory` and one function per op named by `host_fn` (i64 scalars direct; text via ptr-len + stash + read_copy; see `stdlib_design.md` §V.4/§VI.1). Current ABI subset is `(Int, Int) -> Int`; signatures outside the subset or load failures return honest `Err` (no silent skipping; no fabricated hosts). Distinguish by loading method (presence of `host.wasm` in the registry), complementary to standard native hosts. WASM libs are instantiated only when present; standard libs incur zero WASM overhead.
- CLI `sophia run` assembly (`commands::run_with_host`): first `register_wasm_library_hosts` (no-op if none), then register native hosts on demand per entry action’s declared effects (`Http.Get`/`File.*`); third-party WASM ops are often `effectful=false` (not reflected in effect declarations), so their hosts are registered unconditionally.
...
Zero changes to the language core: all steps above do not touch `core/*` or `runtime` code—that is the goal of the library-plugin model. Changes in `core` are only needed to extend the TypeDesc mini-DSL (if a library needs more complex signatures), and must pass the design gate first.
