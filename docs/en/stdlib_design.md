...
> Admission bar: adding a library follows the same demand-driven, design-gate discipline as language expansion—triggered by concrete demo needs, one library per design gate; do not prepopulate a feature list.
...
This purposely covers current library shapes (all `Http`/`File` signatures). It intentionally does not support library-defined param/return types, generics, `one of`, or `list of`—expand the DSL and pass the design gate if/when needed by a future library (YAGNI). This is the basis for switching the semantic effect-op checks from imperative per-op matches to interpreting manifest TypeDescs (table-driven): each library moves from “edit Rust” to “author a manifest.”
...
| Layer | Consumption |
| --- | --- |
| HIR registration (effect symbol table) | `AsgIndex::with_libraries(registry)` injects effect-ops + special-root families |
...
- Third-party WASM libs: `sophia-stdlib::register_wasm_library_hosts(host, registry)` iterates libraries that include `host.wasm` bytes (i.e., WASM-effect libs) and registers a `WasmHostFn` per effect-op—internally holding a `wasmi` instance of `host.wasm` and bridging via the unified byte-level ABI. Distinguishing criterion = loading method (whether `host.wasm` is present in the registry); complementary with standard libs’ native hosts. Current ABI subset `(Int, Int) -> Int` (i64 scalars direct); signatures beyond the subset or load failures honestly return `Err` (no silent skipping, no fabricated hosts), awaiting ABI expansion as needed.
...
Reuse codegen’s byte-level `sophia_host` import ABI (W4’s console_write/file_write/file_read/http_get/read_copy; host only sends/receives linear-memory bytes; no value layout awareness) as the single host contract for both native closures and third-party `host.wasm`—single ABI, already validated, same seam across modes.
...
| `DB` | Persistent data store | (TBD) | (not written) | Future candidate: clarify semantics (KV/relational/document? backend? consistency?) before passing the design gate; not in v1 |
