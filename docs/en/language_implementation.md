...
Standard-library I/O calls (special-root method_call + host delegation; zero new syntax): body-level calls look like `<Root>.<Op>(args)` where `<Root>` is the library’s built-in special-root ident allowed in HIR resolution and not put into ASG index. The type layer merges the corresponding effect and returns types; the interpreter delegates via `HostRegistry`. All I/O libs share this path:

- Http (landed; see `docs/http_lib.md`): `Http.Get(url) → Raw<Text>`; effect merged as `Http.Get` (no arg—capability granularity “may GET”; URLs are usually runtime-bound; see `http_lib.md` §2.6); `url: Text`. Returns untrusted `Raw<Text>`; downstream must convert intents (existing strict-equality checks catch rejects; zero new checks). The interpreter calls a registered `Http.Get` host through `HostRegistry`; mock host provides deterministic buckets (preset url→body; misses → `Err` hard-stop—never fabricate success). Real network is provided by the native host registered through `register_native_hosts` (`reqwest::blocking`); the CLI `run` injects it only if the entry declares `Http.Get`. Real networking is not part of deterministic tests. See `type_layer::infer_effect_op`, `interp::try_effect_op` + `runtime::host`.
- File (landed; see `docs/file_lib.md`): `File.Read(path) → Raw<Text>` / `File.Write(path, Sanitized<Text>) → Unit`; isomorphic to `Http` (special root + effect/capability + intent boundaries + host delegation). The interpreter calls registered `File.Read`/`File.Write` hosts through `HostRegistry`; mock host uses an in-memory bucket (`seed_file`; misses → `Err`). Real file I/O is provided by `register_native_hosts(&mut host, project_root)` (`std::fs`) and is sandboxed under the project root.
...
8. F2 `Http` effect family + host import [from D2]: `Http.Get → Raw<Text>`; like the built-in `Console` family, seeded into `BUILTIN_EFFECT_OPS` (stdlib effect families); reuse existing effect/capability + intent boundaries (zero new syntax) + `HostRegistry` host delegation.
...
- runtime: effect runtime implementations are dispatched by `HostRegistry` by family/op. Executable observable effects: built-in `Console.Write` (triggered by `print`) and stdlib `Http.Get` (body-level `Http.Get(url)`; mock/native hosts are injected through the registry; real networking in `http_lib.md`); `File.Read/Write` with the `File` library (`file_lib.md`).
