...
截至 v0.3，本节面向当前实现以完整归纳已落地且可测试的语言、检查与工具链边界，读者无需回溯旧版本。
...
Sophia 的两阶段流程不是为了制造一个更容易赢 benchmark 的包装器，而是为了把 LLM 的语义设计与形式实现分开。
...
与目标图工作流相关的增量均以可回放与可审计为约束，不改变 Formal Core 的确定性。
---
...
- 场景语义到单任务契约的桥接：workflow 场景（scenario.json）的关键语义转化为统一套件下的基准任务（task.json），避免分裂统计口径。
- 运行方式：
...
- 套件规模：任务数量会随新增任务演进，请使用 experiment list/summarize 查看当前套件大小。
