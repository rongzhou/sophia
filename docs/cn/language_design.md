...
| 允许                                                               | 禁止                                |
| ------------------------------------------------------------------ | ----------------------------------- |
| `let`、`set`、`return`、`raise`、`if/else`、`match`、`repeat N times`、`while cond` | `for`、递归                         |
| 变量、字面量、字段访问、完整 entity 构造                          | lambda、closure、高阶函数           |
| 比较、布尔表达式                                                  | operator overload、隐式转换         |
| `text.length`、`Text + Text`、`text.char_at(index)`、`text.slice(start, length)`、`text.starts_with(prefix)`、`to_text(Int)` | regex、复杂字符串格式化             |
...
| `while cond { ... }`                | cond 必须推断为 `Bool`；每轮重新求值；MVP 不含 `break` / `continue`，不证明终止 |
| `text.char_at(index)`               | `index` 为 Unicode scalar index；负数或越界返回空 `Text`                     |
| `text.slice(start, length)`         | Unicode scalar range；负起点按 0，非正长度或越界起点返回空 `Text`，结束边界夹取 |
| `text.starts_with(prefix)`          | 前缀判断；空前缀返回 `true`                                                  |
...
- `let` / `let mutable` 声明 block-scoped local；`if` / `repeat` / `while` body 创建子作用域。
