...
> 解释器在每次 callable 进入时开一条 span（pre-order）、执行完写回结局，`run_action` 返回的
> `Execution { outcome, host, trace }` 携带完整投影。每条 span 携带 `node_id`（进入的节点）、
...
给出返回类型，解释器经 `EffectHost` 委派。所有 I/O 库共用这一条路径：
...
- **`Http`（已落地，见 `docs/http_lib.md`）**：`Http.Get(url) → Raw<Text>`，并入 effect `Http.Get`
  ...
  解释器经 `EffectHost::http_get` 委派；`InMemoryHost` 用确定性 mock（预置 url→body，未命中即 `Err` 硬
  错误阻断，**绝不伪造成功**）；真实网络由协调层 CLI 的 `CliHost`（`reqwest::blocking`）提供，CLI `run`
  据入口 effect 含 `Http.Get` 才注入；真实网络不进确定性测试。实现见 `type_layer::infer_effect_op`、
  `interp::try_effect_op` + `effect_host`。
- **`File`（已落地，见 `docs/file_lib.md`）**：`File.Read(path) → Raw<Text>` / `File.Write(path,
  ...
  解释器经 `EffectHost::file_read/file_write` 委派；`InMemoryHost` 用内存桶 mock（`seed_file`，未命中
  即 `Err`）；真实文件 IO 由 CLI `CliHost`（`std::fs`）提供。
...
B **不是固定实现序列**，而是由具体演示需求触发、逐项过设计门的最小扩展（详见 `dev_checklist_v1.md`
...
- **runtime**：effect 的运行时实现由 `EffectHost` 按 family/op 分派。可执行的可观测 effect 为内置
  `Console.Write`（`print` 触发）与标准库 `Http.Get`（body 级 `Http.Get(url)` 触发，`InMemoryHost` 用
  确定性 mock，真实网络见 `http_lib.md`）；`File.Read/Write` 随 `File` 库落地（见 `file_lib.md`）。
