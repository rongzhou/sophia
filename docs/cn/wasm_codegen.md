...
> artifact 门禁 + `sophia build`）；**A6（增量查询，与 codegen 解耦）待独立设计门**。
>
> **三段式纪律（用户既定方法论）**：本文是 codegen 的**设计门**——先定输入契约 / 值 ABI / 函数
...
- **不引入 async / 并发 / await / 线程**：起步子集 body 子语言本就是同步纯逻辑（`language_implementation.md`
  §9.3），WASM MVP 的 async 支持有限——这正是当初选 WASM 的理由之一，不在此破例。
...
副作用经 `EffectHost` trait 委派：`console_write` / `file_read` / `file_write` / `http_get`。
`InMemoryHost` 是确定性 mock（`seed_file` / `seed_http` 预置桶，未命中诚实 `Err`）。WASM 中这些是
**host import**（见 §六）。
...
> 若未来 body 子语言显著复杂化（如引入更丰富控制流 / 优化需求），再走设计门评估 body IR。当前不做。
...
| `MethodCall File/Http` | eval args → `call` 对应 host import；失败语义见 §六 |
...
### 6.1 import 接口（与 `EffectHost` trait 对齐）

module 声明以下 import（命名空间 `sophia_host`）：

| import | 签名（值经线性内存 ptr/len 传递） | 对应 `EffectHost` |
| --- | --- | --- |
| `console_write(ptr:i32, len:i32)` | 写一行 | `console_write` |
| `file_read(path_ptr, path_len) -> i32` | 返回 Outcome 句柄（Ok Text / Err） | `file_read` |
...
- **失败语义**：host import 返回 Outcome 句柄；`Err` 物化为 `Raised`（硬错误阻断），与解释器
  `RuntimeError::Validation` 一致——**绝不伪造成功**。
- **capability 边界**：哪些 import 被实际链接由**入口 callable 的 `declared_effects`** 决定（与 CLI
  `needs_real_host` 判定同源）。无 `File.*`/`Http.Get` 的程序不 import 它们（零开销）。capability
  的 allow/deny 在编译期已检查；运行期 host 侧可再加策略（如目录白名单），属 host 职责。

### 6.2 host 侧实现（谁提供 import）

- **差测试 host（进 `cargo test`）**：纯 Rust，复用 `InMemoryHost` 的 mock 语义（`seed_file` /
  `seed_http`）——经 WASM 执行器（§七）的 import 回调桥接到 `InMemoryHost`。这保证差测试与解释器
  **同一份 mock 数据**、同一失败语义。
- **真实部署 host（CLI / 浏览器 / Node，不进门禁）**：CLI 的 `CliHost`（真实 `std::fs` / `reqwest`）
  作为 wasmtime import 提供；其它 host 各自实现 import 接口。这与 S1 / File 库「真实 IO 在协调层、
  不进 cargo test」的既有纪律一致。
...
- **执行（差测试跑 .wasm）**：**采纳** 纯 Rust 的 WASM 执行器（如 `wasmi`——解释型、纯 Rust、无
...
2. WASM 后端 emit + 执行（mock host 注入同一份 seed）→ `Outcome'`；
...
「解释器 vs WASM」的逐 case 比对。这天然覆盖标量 / 结构 / 跨调用 / 错误代数 / `one of` / File·Http。
...
逐字节相等，判据 3）；③ emit `.wasm` 到 `sophia-runs/build/program.wasm`（`sophia.toml` `[build] target`
已切 `wasm`，`out_dir` = `sophia-runs/build`）；④ codegen 未覆盖构造（`to_text`/`List`，无 v1 演示需求）
诚实报 `NotYetImplemented`、**不伪造产出**（解释执行仍可用）。`smoke` 的 build 步随之变为真正 emit。
...
| **W3（A3）** | 差测试夹具：解释器 vs WASM 逐 hidden case 等价，复用 benchmark/e2e 参考解 | `tools/codegen/tests/diff.rs`（或 `runtime`/`cli` 侧） | L1–L5 + D1 等价全绿 ⚙️ **夹具已建**（`tools/codegen/tests/diff.rs`：emit + `wasmi` 执行 + 与解释器 oracle 逐 case 比对；W2–W4 共 19 题等价，**覆盖 benchmark L1–L6〔D1/D2/D3〕+ G2 全部形态**：纯逻辑全形态 + Text/`repeat` + effect〔Console/Http/File + intent 转换〕。仍是逐 case 等价回归，覆盖随新构造扩展） |
| **W4（A4）** | effect host import（console/file/http）+ capability 边界按入口 effect 注入；差测试覆盖 D2（Http）/ D3（File）/ G2（Console）/ G5（File） | codegen import 段 + 差测试 host 桥接 `InMemoryHost` | 含 effect 的题差测试等价 ✅ **已完成**（5 个 `sophia_host` import：console_write / file_write / file_read / http_get / read_copy，字节级 ABI〔host 只处理字节缓冲、不识值布局〕；所有 module 统一声明 import、真实 vs mock 由实例化方选择；差测试经纯 Rust mock host〔seed_file/seed_http 同 `InMemoryHost`，未命中 trap〕跑通 G2 Console / D2 Http+intent / D3 File 往返+intent） |
| **W5（A5）** | strip-assist artifact 字节级比对（扩展 `tools/check`）；`sophia build` emit `.wasm`；`smoke` 串通 | `tools/check/src/strip_assist.rs` + `cli/src/commands.rs` | 字节 diff 门禁 + build 产出 artifact ✅ **已完成**（artifact 门禁落 `tools/codegen`〔`check_artifact_strip_equivalence`，emit 字节确定 → 移除 assist 前后逐字节相等〕；`sophia build` check→门禁→emit `program.wasm`，未覆盖构造诚实报 `NotYetImplemented`；smoke 串通） |
| **A6** | 增量查询架构（Salsa 思想，支撑 LSP）——**与 codegen 解耦、可并行 / 推后**，不在本设计门细化 | —— | 独立设计门（按需） |
...
> 七项已全部确认采纳，本文从「设计门草案」转「已定稿」，按 §九 W1→W5 推进；每阶段同步
...
- 2026-05-31 — 初稿（设计门草案，待讨论确认）。定位工作流 A 为 v1 完成判据 1（差测试等价）+ 判据 3
  ...
  纯文档，无代码改动。
- 2026-05-31 — **定稿（七个决策点全部采纳）**。设计门讨论确认：① body 不引入新 lowered IR（直接遍历
...
- 2026-05-31 — **W4 落地（A4 effect host import：Console / File / Http）**。emit 把副作用映射为 WASM
  **host import**（§六）：5 个 `sophia_host` import——`console_write(ptr,len)` / `file_write(pp,pl,cp,cl)` /
  ...
  未命中 trap），解释器 oracle 改 `run_action_with_host` 注入同一份 seed。全工作区 **358 passed / 0 failed**，
...
- 2026-05-31 — **W5 落地（A5 strip-assist artifact 层 + `sophia build` emit；工作流 A W1–W5 收尾）**。
  ① `tools/codegen` 新增 `emit_from_sources(sources, strip)` + `check_artifact_strip_equivalence`——移除全部
  ...
  更新。`tools/codegen` 加 `sophia-hir` 依赖（`emit_from_sources` 需 `resolve_program`），CLI 加 `sophia-codegen`
  依赖。codegen 测试 + 确定性 2 题、CLI pipeline build emit + 未覆盖诚实报告 2 题。全工作区 **362 passed /
  ...
  独立设计门；真实部署 host（wasmtime / 浏览器）随实际部署需求接入；`to_text`/`List` 按需再补（YAGNI）。