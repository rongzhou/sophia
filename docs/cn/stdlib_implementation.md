...
`core` **不依赖 `sophia-stdlib`**：`core/hir` 的 `AsgIndex::new(&registry)/AsgIndex::build(inputs, &registry)` 与 `core/semantic`
...
| 触点 | 实现 |
| --- | --- |
| HIR effect 符号表 + 特殊根 | `AsgIndex::new(registry)/AsgIndex::build(inputs, registry)` 把 effectful op 灌入 `effect_ops`（arity=0）+ family 灌入 `library_families` + op 契约灌入 `library_ops`（均 `#[serde(skip)]` 派生符号表） |
...
| 运行时分派 | 解释器 `try_effect_op` 用 `host.has_op(family, op)` 判定特殊根 effect op、`host.call(family, op, args)` 委派 |
| codegen | `CodegenInput` 持 `lib_index`（`AsgIndex::new(registry)`）供 emit 重算 TypeTable；host import 名 = `host_fn` |
...
`AsgIndex.library_domains`（`registry 显式参数从 `registry.sophia_sources()` 填充）+
...
> 只在「扩 TypeDesc mini-DSL」（某库需更复杂签名）时才需要，且须先完成设计评审。
...
- 2026-05-31 — **库插件 P1 落地（消化原 `library_plugin.md`）**。重写为清单 → 注册表 → 各层消费的实现
  ...
  `AsgIndex::new(registry)` / `AsgIndex::build(inputs, registry)` 注入库 effect/特殊根/op 契约；`type_layer::infer_effect_op` 表驱动化
  （TypeDesc → Ty）；host 采用 `HostRegistry: (family,op) → Box<dyn HostFn>`（`runtime` 不内置具体库）；
